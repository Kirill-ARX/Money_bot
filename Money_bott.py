...
# =====================================================================
# НАСТРОЙКИ: Впиши сюда данные
# =====================================================================
TOKEN = "8628691428:AAEZ6Ec44uHwm5ey5xsIS6Ilj-9dNuDYOas"  # Токен бота

ADMIN_ID = 5551943786  # Твой Telegram ID (ты будешь админом и юзером)
USER2_ID = 5178460435  # Telegram ID второго партнера
USER3_ID = 5959142753  # Telegram ID третьего партнера
...
# Подключаемся к базе данных (файл wallet.db создастся сам в этой же папке)
db = sqlite3.connect("wallet.db")
cursor = db.cursor()

# Создаем таблицы, если их еще нет
# Таблица пользователей и их балансов
cursor.execute("""
...
""")
# Таблица глобальных настроек (общая прибыль и процент Озона)
cursor.execute("""
...
def get_main_keyboard(user_id):
    ...
    # Если кнопку нажимает админ (ты) — добавляем ему кнопку админки
    if user_id == ADMIN_ID:
...
async def ozon_calc(message: Message):
    ...
    # Считаем доход (Простая формула: Баланс * % / 100)
...
# Шаблон для изменения балансов (чтобы код не раздувать, управляем через команды текстом)
...
async def info_how_to_change(message: Message):
    ...
    await message.answer(
        ...
        "`сет баланс 3 -1000` (уменьшит баланс Лёне на 1000)\n"
        "`сет админ 10000` (изменит тебе баланс)\n"
...
# Обработчик текстовых команд управления (парсинг текста вроде "сет баланс 2 50000")
...
async def admin_commands(message: Message):
    ...
    try:
        ...
        if cmd_type == "баланс":
            ...
            await message.answer(f"✅ Баланс {parts[2]} изменен на {amount} руб.")
        ...
        await message.answer("❌ Ошибка в формате команды. Проверь пробелы и цифры.")

# =====================================================================
# РАБОТА В ГРУППЕ: ПРИВЕТСТВИЯ И КОМАНДЫ ДЛЯ ТЕМЫ
# =====================================================================

# 1. ХЕНДЛЕР ПРИВЕТСТВИЯ (срабатывает, когда добавляют нового человека)
@dp.message(F.new_chat_members)
async def welcome_new_member(message: Message):
    for member in message.new_chat_members:
        text = (
            f"🌟 *Доброго времени суток, {member.mention_markdown()}!*\n\n"
            f"🤝 Вы все под моим информированием.\n"
            f"Бот успешно синхронизирован. Можно пользоваться кнопками в личке или командами здесь."
        )
        await message.answer(text, parse_mode="Markdown")


# 2. КОМАНДА: /stat (работает в группе через фильтр Command)
@dp.message(Command("stat"))
async def group_stat_cmd(message: Message):
    ...
    text = (
        f"📊 *ОТЧЕТ ПО ТЕМКЕ В ЧАТЕ* 📊\n\n"
        f"💰 *Всего денег в пуле:* {total_balances:,} руб.\n"
        f"📈 *Чистая прибыль:* {profit:,} руб.\n"
        f"🏦 *Ставка Ozon:* {ozon_pct}%\n\n"
        f"👥 *Текущий баланс участников:*\n{users_info}"
    )
    await message.answer(text, parse_mode="Markdown")


# 3. КОМАНДА: /id (вернет ID чата и топика)
...
# 4. КОМАНДА: /calculator
@dp.message(Command("calculator"))
async def group_calculator_cmd(message: Message):
    if message.from_user.id not in ALLOWED_USERS: return
