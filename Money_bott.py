...
async def get_chat_and_user_id(message: Message):
    ...
    text = (
        ...
        f"📌 *ID текущей темы (топика):* `{thread_id if thread_id else 'Основной chat'}`"
    )
    await message.answer(text, parse_mode="Markdown")

# =====================================================================
# АДМИНКА (ФИКС НУЛЯ: Сет жестко ставит значение, а не прибавляет)
# =====================================================================
...
async def info_how_to_change(message: Message):
    if message.from_user.id != ADMIN_ID: return
    await message.answer(
        "ℹ️ Чтобы изменить баланс или прибыль, отправь сообщение в формате:\n\n"
        "`сет баланс 2 0` (скинет баланс Максима в чистый ноль)\n"
        "`сет баланс 3 15000` (поставит баланс Лёне ровно 15к)\n"
        "`сет админ 5000` (поставит тебе баланс 5000)\n"
        "`сет прибыль 1000` (установит чистую прибыль)\n",
        parse_mode="Markdown"
    )
...
async def admin_commands(message: Message):
    ...
    try:
        ...
        await message.answer("❌ Ошибка в формате. Пример: `сет баланс 2 0`")

# =====================================================================
# КОД-БУДИЛЬНИК (Обман таймаута Render)
# =====================================================================
...
async def start_web_server():
    ...
    await runner.setup()
    # Бесплатный Render сам передает нужный порт в переменную среды PORT
...
# =====================================================================
async def main():
    # Запускаем встроенный будильник, чтобы Render видел открытый порт
